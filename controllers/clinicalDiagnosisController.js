...
const clinicalDiagnosisCRUD = async (req, res) => {
  try {
    ...
    const result = await pool.query('CALL proc_clinical_diagnosis_crud($1, NULL)', [JSON.stringify(params)]);
    ...
    if (action_mode === 'insert') {
      ...
      res.json({ status: 'success', message: 'Record inserted successfully', data: insertedData.rows[0] });
    } else {
      res.json({ status: 'success', message: `Record ${action_mode}d successfully` });
    }
  } catch (error) {
...
const getClinicalDiagnosisById = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('SELECT * FROM t_clinical_diagnosis WHERE id = $1', [id]);
