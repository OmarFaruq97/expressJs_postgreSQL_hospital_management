...
const coMorbiditiesCRUD = async (req, res) => {
  try {
    ...
    const result = await pool.query('CALL proc_co_morbidities_crud($1, NULL)', [JSON.stringify(params)]);
...
const getCoMorbidityById = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('SELECT * FROM t_co_morbidities WHERE id = $1', [id]);
